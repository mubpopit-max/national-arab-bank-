...
def init_db():
    ...
    database.executescript(
        ...
        CREATE INDEX IF NOT EXISTS idx_external_reference
        ON external_transfers(reference);


        CREATE INDEX IF NOT EXISTS idx_external_idempotency
...
def generate_reference():
    ...
    for _ in range(100):
        ...
        if not exists:
            ...
            external_exists = database.execute(
                """
                SELECT 1
...
def call_digital_store(
    ...
    if not DIGITAL_STORE_API_BASE_URL:
        ...
        raise RuntimeError(
            "DIGITAL_STORE_API_BASE_URL غير مضبوط."
        )

    if not DIGITAL_STORE_API_KEY:

        raise RuntimeError(
            "DIGITAL_STORE_API_KEY غير مضبوط."
        )
    ...
    try:
        ...
        raise RuntimeError(
            "تعذر الاتصال بمنصة DIGITAL STORE."
        ) from error

    except TimeoutError as error:

        raise RuntimeError(
            "انتهت مهلة الاتصال بمنصة DIGITAL STORE."
        ) from error
        ...
        if action == "execute":
            ...
            try:
                ...
                if not re.fullmatch(
                    ...
                    raise ValueError(
                        "رقم حساب DIGITAL STORE غير صالح."
                    )


                # ------------------------------------------------
                # منع إعادة تنفيذ نفس المفتاح
                # ------------------------------------------------

                old_external = database.execute(
                    """
                    SELECT *
                    FROM external_transfers
                ...
                if old_external:
                    ...
                    if old_external["status"] == "COMPLETED":
                    ...
                    if old_external["status"] == "PENDING":

                        flash(
                            "هذه العملية قيد المعالجة بالفعل."
                        )
                ...
                # ------------------------------------------------
                # التحقق من الحساب في DIGITAL STORE
                # ------------------------------------------------

                lookup_result = call_digital_store(
                    "GET",
                    (
                        "/api/v1/accounts/lookup"
                        "?account_number="
                ...
                if not lookup_result.get(
                    ...
                    raise ValueError(
                        lookup_result.get(
                            "error",
                            "حساب DIGITAL STORE غير موجود.",
                        )
                    )


                receiver_obj = lookup_result[
                    "account"
                ]


                if not receiver_obj.get(
                    "active",
                    False,
                ):

                    raise ValueError(
                        "حساب DIGITAL STORE غير نشط."
                    )
                ...
                # ------------------------------------------------
                # خصم البنك وتسجيل PENDING
                #
                # لا نحتفظ بقفل SQLite أثناء HTTP.
                # ------------------------------------------------
                ...
                # ------------------------------------------------
                # إرسال العملية إلى DIGITAL STORE
                # ------------------------------------------------

                try:

                    store_result = call_digital_store(
                        "POST",
                ...
                except Exception as api_error_exception:

                    # ------------------------------------------------
                    # محاولة معرفة حالة العملية قبل أي استرجاع.
                    #
                    # لا نعيد المبلغ تلقائيًا إذا كان من الممكن
                    # أن يكون DIGITAL STORE قد نفذ العملية بالفعل.
                    # ------------------------------------------------

                    try:

                        status_result = call_digital_store(
                            "GET",
                            (
                                "/api/v1/transfers/"
                                + reference
                    ...
                    flash(
                        "تعذر تأكيد التحويل مع DIGITAL STORE. "
                        "تم تعليق العملية ولم يتم اعتبارها مكتملة."
                    )
                ...
                if not sender_check:
                    ...
                    raise RuntimeError(
                        "تعذر التحقق من رصيد البنك."
                    )


                if int(
                    sender_check["balance"]
                ) != sender_after:

                    raise RuntimeError(
                        "حدث اختلاف في رصيد البنك."
                    )
...
def api_digital_store_inbound_transfer():

    if not verify_digital_store_api_key():
    ...
    try:
        ...
        if not receiver:
            ...
            raise ValueError(
                "الحساب البنكي غير موجود أو غير نشط."
            )
        ...
        if int(
            receiver_check["balance"]
        ) != receiver_after:

            raise ValueError(
                "حدث اختلاف في الرصيد."
            )
        ...
        database.execute(
            ...
            (
                ...
                receiver_before,
                receiver_after,
                "DIGITAL_STORE_INBOUND",
                "COMPLETED",
                "تحويل وارد من DIGITAL STORE 249",
                "",
                now(),
            ),
        )
        ...
        return api_error(
            "العملية موجودة بالفعل أو حدث تعارض.",
            409,
        )
